...
import loadCustomers from 'src/data/customer';
import search from 'src/helpers/search';
import { sortByHighestNumber } from 'src/helpers/sort';
...
export default class CustomerResolver {
  ...
  async customers(
    ...
    if (sortBy === Contanst.HIGHEST_TO_LOWEST) {
      customers = sortByHighestNumber(customers, 'total_order_amount');
    }
    if (sortBy === Contanst.LOWEST_TO_HIGHEST) {
      customers = sortByHighestNumber(customers, 'total_order_amount');
    }
  ...
  @Query(() => Customer)
  async customer(@Arg('id', (type) => ID) id: string): Promise<Customer | undefined> {
    // as auth Customer. check from middleware.
    console.log(id, 'customer_id');
    return this.customersCollection.find((customer) => customer.id === id);
  }
