import loadStuffs from 'src/data/stuff';
import { transKeys } from 'src/helpers/i18n';
import search from 'src/helpers/search';
import AddStuffInput from 'src/types/inputStuff';
import Stuff from 'src/types/stuff';
...
export default class StuffResolver {
  ...
  async stuff(@Arg('id', (type) => ID) id: string): Promise<Stuff | undefined> {
    ...
    return this.stuffsCollection.find((stuff) => stuff.id === id);
  }

  @Mutation(() => Stuff, { description: transKeys.create_stuff })
